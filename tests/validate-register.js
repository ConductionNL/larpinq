...
const VALID_STRING_FORMATS = new Set([
	...
	'relative-json-pointer', 'regex', 'url',
	// OpenRegister additions.
	'color', 'color-hex', 'color-hex-alpha', 'color-rgb', 'color-rgba',
	'color-hsl', 'color-hsla', 'semver',
])
...
function checkFormats(node, at, errors, propPath) {
	...
	if (typeof node.format === 'string' && !VALID_STRING_FORMATS.has(node.format)) {
		const isStringish = node.type === 'string'
...
function validateFragments(errors) {
	...
	if (!fs.existsSync(dir)) return
	for (const entry of fs.readdirSync(dir).filter((f) => f.endsWith('.json')).sort()) {
