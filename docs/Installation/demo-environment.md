...
This page gets a working Larpinq running on your own machine in two commands. You end with a character and campaign registry.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/larpinq/development/larpinq-compose.yaml
```
...
==> installing larpinq <version>
==> apps present: integriq larpinq openregister thematiq
```
...
```bash
curl -s http://localhost:8607/status.php
```
...
| What | Where |
| --- | --- |
| **Larpinq** | [http://localhost:8607/apps/larpinq/](http://localhost:8607/apps/larpinq/) |
| Admin interface | [http://localhost:8607](http://localhost:8607) — `admin` / `admin` |
...
| `larpinq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing larpinq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8607/apps/larpinq/"
...
curl -s -u admin:admin "http://localhost:8607/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f larpinq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f larpinq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f larpinq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
