...
Create a new character in Larpinq, fill in the core fields (name, background, faith, starting currency), and submit it. Stats are computed automatically once you attach skills, items, conditions, and events later.

## Goal

By the end you will have a character record in Larpinq with a name, type (player or NPC), a background story, and a starting currency. The character will be visible in the **Characters** list and on the dashboard's *Recent characters* widget.

## Prerequisites

- Larpinq open and the OpenRegister back end connected (see [Open Larpinq for the first time](01-first-launch.md)).
...
1. Open **Characters** from the Larpinq navigation and click **Add Item**. The *Create Item* dialog opens.
...
| Symptom | Fix |
|---|---|
| **Add Item** opens an empty dialog | The `character` schema isn't imported — ask an admin to re-run the register configuration (**Settings → Administration → Larpinq**, see [Manage Larpinq settings](../admin/03-admin-settings.md)). |
