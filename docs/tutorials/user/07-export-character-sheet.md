...
- A character that exists in Larpinq with skills, items, and any other relations attached (see [Build a character's skills and abilities](03-character-skills.md)).
- The **DocuDesk** app installed and enabled on the same Nextcloud instance — Larpinq delegates the PDF rendering to it. Without DocuDesk the export endpoint returns a 424 *Failed Dependency* and the **Download PDF** button is hidden.
- At least one character-sheet **template** registered in DocuDesk and scoped to Larpinq. Templates are HTML with Twig placeholders (`{{ character.name }}`, `{% for skill in character.skills %}…`); the game master maintains them in DocuDesk.
...
4. Confirm. Larpinq packages the full character context — properties, computed abilities, skills, items, conditions, events, currency — and hands it to DocuDesk's PDF service. The browser downloads the file as `<characterName>_character_sheet.pdf`.
...
| Symptom | Fix |
|---|---|
| No **Download PDF** button on the character | DocuDesk is not installed or not enabled — install it from the app store, then reload the character page. The Larpinq endpoint returns HTTP 424 when DocuDesk is absent; the UI hides the action accordingly. |
| Template picker is empty | No Larpinq-scoped templates have been registered in DocuDesk — ask the game master to add at least one. |
