OC.L10N.register(
    "larpinq",
    {
        "Load example data?": "Ar įkelti pavyzdinius duomenis?",
        "Example data fills the lists, detail pages and dashboards so you can see the app working straight away. Pick \"None\" on a production install.": "Pavyzdiniai duomenys užpildo sąrašus, detalių puslapius ir skydelius, todėl programėlę iškart pamatysite veikiančią. Gamybinėje diegtyje pasirinkite \"Nėra\".",
        "Load the example data": "Įkelti pavyzdinius duomenis",
        "Loads what you picked. The data is obviously sample data, it is safe to run more than once, and you can delete it afterwards.": "Įkelia tai, ką pasirinkote. Tai akivaizdžiai pavyzdiniai duomenys, veiksmą galima saugiai kartoti, o vėliau juos galima ištrinti.",
        "None, I will set this up myself": "Nėra, susitvarkysiu pats",
        "Nothing is imported. You start with an empty app and add your own data.": "Niekas neimportuojama. Pradedate nuo tuščios programėlės ir pridedate savo duomenis.",
        "Example data": "Pavyzdiniai duomenys",
        "Sample values for every schema this app supplies, generated from the schemas themselves. It shows the lists, detail pages and dashboards working rather than telling a story. Safe to run more than once, and you can delete it afterwards.": "Pavyzdinės reikšmės kiekvienai schemai, kurią pateikia ši programėlė, sugeneruotos iš pačių schemų. Jos rodo sąrašus, detalių puslapius ir skydelius veikiant, o ne pasakoja istoriją. Saugu kartoti ir vėliau ištrinti.",
        "Larpinq": "Larpinq",
        "Dashboard": "Skydelis",
        "Characters": "Personažai",
        "Character": "Personažas",
        "Players": "Žaidėjai",
        "Player": "Žaidėjas",
        "Abilities": "Gebėjimai",
        "Ability": "Gebėjimas",
        "Skills": "Įgūdžiai",
        "Skill": "Įgūdis",
        "Items": "Daiktai",
        "Item": "Daiktas",
        "Conditions": "Būsenos",
        "Condition": "Būsena",
        "Effects": "Efektai",
        "Effect": "Efektas",
        "Events": "Renginiai",
        "Event": "Renginys",
        "Settings": "Nustatymai",
        "Worlds": "Žaidimo pasauliai",
        "Setting": "Nustatymas",
        "World": "Žaidimo pasaulis",
        "Documentation": "Dokumentacija",
        "New character": "Naujas personažas",
        "New player": "Naujas žaidėjas",
        "New ability": "Naujas gebėjimas",
        "New skill": "Naujas įgūdis",
        "New item": "Naujas daiktas",
        "New condition": "Nauja būsena",
        "New effect": "Naujas efektas",
        "New event": "Naujas renginys",
        "Name": "Pavadinimas",
        "Description": "Aprašymas",
        "Type": "Tipas",
        "Approved": "Patvirtinta",
        "Gold": "Auksas",
        "Silver": "Sidabras",
        "Copper": "Varis",
        "Base": "Pagrindas",
        "Modifier": "Modifikatorius",
        "Modification": "Modifikacija",
        "Cumulative": "Kaupiamasis",
        "Location": "Vieta",
        "Start date": "Pradžios data",
        "End date": "Pabaigos data",
        "Unique": "Unikalus",
        "Value": "Reikšmė",
        "player": "žaidėjas",
        "npc": "npc",
        "other": "kita",
        "no": "ne",
        "approved": "patvirtinta",
        "positive": "teigiamas",
        "negative": "neigiamas",
        "cumulative": "kaupiamasis",
        "non-cumulative": "nekaupiamasis",
        "Save": "Įrašyti",
        "Cancel": "Atšaukti",
        "Delete": "Ištrinti",
        "Edit": "Redaguoti",
        "Search": "Ieškoti",
        "Loading...": "Įkeliama...",
        "Back to list": "Atgal į sąrašą",
        "Are you sure you want to delete this?": "Ar tikrai norite tai ištrinti?",
        "Download PDF": "Atsisiųsti PDF",
        "Select a template to generate a PDF from this character": "Pasirinkite šabloną, kad sugeneruotumėte PDF iš šio personažo",
        "PDF generation requires the DocuDesk app to be installed and enabled": "PDF generavimui reikia, kad programa DocuDesk būtų įdiegta ir įjungta",
        "Configuration saved": "Konfigūracija įrašyta",
        "Configuration re-imported successfully": "Konfigūracija sėkmingai importuota iš naujo",
        "Re-import configuration": "Importuoti konfigūraciją iš naujo",
        "Game setup": "Žaidimo sąranka",
        "Advanced settings": "Išplėstiniai nustatymai",
        "Configure OpenRegister data source to enable this widget": "Sukonfigūruokite OpenRegister duomenų šaltinį, kad įjungtumėte šį valdiklį",
        "Loading skill data...": "Įkeliami įgūdžių duomenys...",
        "Retry": "Bandyti dar kartą",
        "No skill data available": "Nėra įgūdžių duomenų",
        "characters": "personažai",
        "Other": "Kita",
        "Failed to load skill data": "Nepavyko įkelti įgūdžių duomenų",
        "Data storage": "Duomenų saugykla",
        "Configure where to store your LARP data": "Sukonfigūruokite, kur saugoti savo LARP duomenis",
        "Open Register is not installed. Some features might be unavailable.": "Open Register neįdiegtas. Kai kurios funkcijos gali būti nepasiekiamos.",
        "Source": "Šaltinis",
        "Register": "Registras",
        "Schema": "Schema",
        "Internal": "Vidinis",
        "Open Register": "Open Register",
        "Yes": "Taip",
        "No": "Ne",
        "Create": "Sukurti",
        "No characters yet": "Personažų dar nėra",
        "No events yet": "Renginių dar nėra",
        "View all ({count})": "Žiūrėti visus ({count})",
        "Recent characters": "Naujausi personažai",
        "Recent events": "Naujausi renginiai",
        "Skill usage by characters": "Įgūdžių naudojimas pagal personažus",
        "Refresh dashboard": "Atnaujinti skydelį",
        "Larpinq settings": "Larpinq nustatymai",
        "Configure your Larpinq installation": "Sukonfigūruokite savo Larpinq įdiegtį",
        "Version information": "Versijos informacija",
        "Information about the current Larpinq installation": "Informacija apie dabartinę Larpinq įdiegtį",
        "Support": "Pagalba",
        "For support, contact us at": "Norėdami gauti pagalbą, susisiekite su mumis",
        "For a Service Level Agreement (SLA), contact": "Dėl paslaugų lygio sutarties (SLA) susisiekite su",
        "Importing...": "Importuojama...",
        "Re-import failed": "Pakartotinis importavimas nepavyko",
        "Configuration": "Konfigūracija",
        "OpenRegister is not configured. Some features may be limited.": "OpenRegister nesukonfigūruotas. Kai kurios funkcijos gali būti ribotos.",
        "Configure": "Konfigūruoti",
        "Failed to save. Please try again.": "Nepavyko įrašyti. Bandykite dar kartą.",
        "Failed to delete.": "Nepavyko ištrinti.",
        "Background": "Fonas",
        "Save all": "Įrašyti viską",
        "Unnamed character": "Bevardis personažas",
        "Unnamed event": "Bevardis renginys",
        "Welcome to Larpinq!": "Sveiki atvykę į Larpinq!",
        "Configure your Larpinq settings here.": "Čia sukonfigūruokite savo Larpinq nustatymus.",
        "Failed to save settings": "Nepavyko įrašyti nustatymų",
        "General": "Bendra",
        "Larpinq Settings": "Larpinq nustatymai",
        "Save All": "Įrašyti viską",
        "Settings saved successfully": "Nustatymai sėkmingai įrašyti",
        "Version Information": "Versijos informacija",
        "Check-in": "Registracija",
        "Attendance": "Dalyvavimas",
        "Check in": "Registruoti atvykimą",
        "No-show": "Neatvyko",
        "Registered": "Užsiregistravęs",
        "Checked in": "Atvykęs",
        "No confirmed participants for this event yet.": "Šiam renginiui dar nėra patvirtintų dalyvių.",
        "Attendance tracking is unavailable — showing the participant list read-only.": "Dalyvavimo apskaita neprieinama — dalyvių sąrašas rodomas tik skaitymui.",
        "Skill tree": "Įgūdžių medis",
        "No character (uncoloured)": "Nėra personažo (nespalvinta)",
        "All worlds": "Visi žaidimo pasauliai",
        "Owned": "Turimas",
        "Available": "Prieinamas",
        "Locked": "Užrakintas",
        "Unknown": "Nežinomas",
        "No skills to show": "Nėra įgūdžių, kuriuos būtų galima rodyti",
        "No skills exist yet for the selected world.": "Pasirinktam žaidimo pasauliui dar nėra jokių įgūdžių.",
        "Requires: {list}": "Reikalauja: {list}",
        "No prerequisites": "Nėra būtinų sąlygų",
        "Required skills": "Reikalingi įgūdžiai",
        "Required abilities": "Reikalingi gebėjimai",
        "(score ≥ {n})": "(rezultatas ≥ {n})",
        "Required conditions": "Reikalingos būsenos",
        "Required effects": "Reikalingi efektai",
        "Prerequisites": "Būtinos sąlygos",
        "None — this is a root skill.": "Nėra — tai pagrindinis įgūdis.",
        "Where the automation lives": "Kur gyvena automatizavimas",
        "Flows are what happens without anyone clicking: a reminder before a deadline passes, a confirmation sent on submission. This is where you read and edit them. Nothing to build now.": "Flows yra tai, kas vyksta niekam nespaudžiant: priminimas prieš pasibaigiant terminui, patvirtinimas pateikus. Čia juos skaitai ir redaguoji — dabar nieko kurti nereikia.",
        "Open Flows in the menu": "Atverkite Flows meniu",
        "Ability Name": "Gebėjimo pavadinimas",
        "Affected Characters": "Paveikti personažai",
        "Amount of copper pieces": "Varinių monetų skaičius",
        "Amount of gold pieces": "Auksinių monetų skaičius",
        "Amount of silver pieces": "Sidabrinių monetų skaičius",
        "Automatic system notices": "Automatiniai sistemos pranešimai",
        "Award Reason": "Skyrimo priežastis",
        "Awarded At": "Skirta",
        "Awarded By": "Skyrė",
        "Background Story": "Kilmės istorija",
        "Base Value": "Bazinė reikšmė",
        "Character Card": "Personažo kortelė",
        "Character Name": "Personažo vardas",
        "Checked In At": "Atvykimas užregistruotas",
        "Checked In By": "Atvykimą užregistravo",
        "Condition Name": "Būsenos pavadinimas",
        "Contact email address": "Kontaktinis el. pašto adresas",
        "Copper Pieces": "Varinės monetos",
        "Effect Name": "Efekto pavadinimas",
        "End Date": "Pabaigos data",
        "Event Name": "Renginio pavadinimas",
        "Event description": "Renginio aprašymas",
        "Event end date and time": "Renginio pabaigos data ir laikas",
        "Event location": "Renginio vieta",
        "Event name": "Renginio pavadinimas",
        "Event start date and time": "Renginio pradžios data ir laikas",
        "Faith": "Tikėjimas",
        "Full name of the player": "Visas žaidėjo vardas",
        "Game Master Notes (Private)": "Žaidimo vedėjo pastabos (privačios)",
        "Game Master Notes (Public)": "Žaidimo vedėjo pastabos (viešos)",
        "Gold Pieces": "Auksinės monetos",
        "Item Name": "Daikto pavadinimas",
        "Items and Money": "Daiktai ir pinigai",
        "Mechanical Effect": "Žaidimo efektas",
        "Modifier Value": "Modifikatoriaus reikšmė",
        "Name of the condition": "Būsenos pavadinimas",
        "Name of the effect": "Efekto pavadinimas",
        "Name of the event": "Renginio pavadinimas",
        "Name of the item": "Daikto pavadinimas",
        "Name of the skill": "Įgūdžio pavadinimas",
        "Name of the stat": "Savybės pavadinimas",
        "Nextcloud user": "Nextcloud naudotojas",
        "Notes about items and money": "Pastabos apie daiktus ir pinigus",
        "Notes about the player": "Pastabos apie žaidėją",
        "Overridden At": "Išimtis suteikta",
        "Overridden By": "Išimtį suteikė",
        "Override Reason": "Išimties priežastis",
        "Owner": "Savininkas",
        "Owner UID": "Savininko UID",
        "Participating Characters": "Dalyvaujantys personažai",
        "Player Name": "Žaidėjo vardas",
        "Post-Event Effects": "Efektai po renginio",
        "Real name of the player": "Tikrasis žaidėjo vardas",
        "Required Conditions": "Reikalingos būsenos",
        "Required Effects": "Reikalingi efektai",
        "Required Score": "Reikalinga reikšmė",
        "Required Skills": "Reikalingi įgūdžiai",
        "Required Stats": "Reikalingos savybės",
        "Requirement Overrides": "Būtinų sąlygų išimtys",
        "Setting Name": "Žaidimo pasaulio pavadinimas",
        "Silver Pieces": "Sidabrinės monetos",
        "Skill Name": "Įgūdžio pavadinimas",
        "Start Date": "Pradžios data",
        "Starting value for all characters": "Pradinė reikšmė visiems personažams",
        "Stat": "Savybė",
        "Status": "Statusas",
        "System Notice": "Sistemos pranešimas",
        "Unique Artifact": "Unikalus artefaktas",
        "Unique Condition": "Unikali būsena",
        "XP Amount": "Patirties taškų skaičius",
        "XP Award": "Patirties taškų skyrimas",
        "Reports": "Ataskaitos",
        "Pick a report to open it.": "Pasirinkite ataskaitą, kad ją atidarytumėte.",
        "Open": "Atviras",
        "In progress": "Vykdoma",
        "Blocked": "Užblokuota",
        "Date": "Data",
        "Due": "Terminas",
        "Assignee": "Priskirta",
        "Who": "Kas",
        "What": "Kas",
        "Minutes": "Minutės",
        "Entries": "Įrašai",
        "Most recent": "Naujausi",
        "Per person": "Vienam asmeniui",
        "By status": "Pagal būseną",
        "By priority": "Pagal prioritetą",
        "Character roster": "Personažų sąrašas",
        "Progression": "Progresas",
        "World content": "Pasaulio turinys",
        "Awaiting approval": "Laukia patvirtinimo",
        "Player characters": "Žaidėjų personažai",
        "Awards": "Skyrimai",
        "Experience": "Patirtis",
        "Experience awarded": "Suteikta patirtis",
        "Per character": "Vienam personažui",
        "By type": "Pagal tipą",
        "By approval": "Pagal patvirtinimą",
        "Items carried by characters": "Personažų nešami daiktai",
        "Conditions on characters": "Personažų būsenos",
        "Nothing awarded yet": "Dar nieko nesuteikta",
        "Who is playing what, and what is still waiting for approval.": "Kas ką žaidžia ir kas dar laukia patvirtinimo.",
        "Experience awarded, and who earned it.": "Suteikta patirtis ir kas ją užsidirbo.",
        "How much the world holds, and what characters actually carry.": "Kiek pasaulis apima ir ką personažai iš tikrųjų neša.",
        "Store": "Parduotuvė",
        "Install game configurations that other organisations have published: a skill tree, a set of conditions, or the flows behind an event.": "Įdiekite registrus, schemas ir srautus, kuriuos paskelbė kitos organizacijos."
    },
    "nplurals=2; plural=(n != 1);"
)
