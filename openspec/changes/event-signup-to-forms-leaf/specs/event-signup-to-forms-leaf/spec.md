...
# Event Sign-up via Forms Leaf

## Purpose

Implement the planned event sign-up / waiting-list intake by consuming the
OpenRegister forms integration leaf on the Event detail page, per ADR-019 +
ADR-022, instead of building a bespoke sign-up form and submission store. The
event-domain capacity/waitlist rules stay in LarpingApp.
