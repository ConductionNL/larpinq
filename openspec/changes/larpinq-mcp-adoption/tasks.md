...
### Task 1: Add the `register.d/larpinq-mcp-adoption.json` fragment, including the three bounded write verbs
...
- **files**: `lib/Settings/register.d/larpinq-mcp-adoption.json`
- **acceptance_criteria**:
  - GIVEN the exact JSON in design.md WHEN it is written to `lib/Settings/register.d/larpinq-mcp-adoption.json` THEN all 9 curated schemas carry a `configuration.x-openregister-mcp` block with `enabled: true`
...
- **files**: `lib/Settings/register.d/larpinq-mcp-adoption.json`, `lib/Settings/larpinq_register.json` (read-only reference), `lib/Settings/register.d/event-checkin-roster.json` (read-only reference for `attendance`)
- **acceptance_criteria**:
