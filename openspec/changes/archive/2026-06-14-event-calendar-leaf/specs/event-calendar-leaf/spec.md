...
Surface the OpenRegister calendar integration leaf on the Event detail page so
that an Event's `startDate`/`endDate` range is visualised and managed as a
calendar event, instead of LarpingApp building a bespoke calendar/scheduling
UI. Consumes the OR integration registry (ADR-019) per ADR-022.
