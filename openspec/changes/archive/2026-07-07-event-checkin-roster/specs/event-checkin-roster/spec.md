...
# Event Check-in Roster — Delta

**Spec refs**: ADR-001 (all data via OpenRegister — no app-local tables), ADR-022 (apps consume OR abstractions; authorization is OR-delegated), ADR-037 (modular register fragments), the cross-app schema-slug collision rule (global `lower(slug)` — namespace collision-prone slugs). Adjacent: `event-xp-awards` (attendance is the eligibility signal), `event-signup-to-forms-leaf` (confirmed sign-ups are the roster), `pdf-export` (run-sheet).
