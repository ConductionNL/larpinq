...
# Event Location via Maps Leaf

## Purpose

Replace the Event's free-text `location` string with the OpenRegister maps
integration leaf (structured location data + map widget) on the Event detail
page, consuming the OR integration registry (ADR-019) per ADR-022 instead of
building a bespoke geo model or map embed.
