...
`OCA\Larpinq\Portal\PortalContributionProvider` that declares — for the
...
- Larpinq keeps ownership of its data and domain logic; it only *declares*
...
- **No inbox** — Larpinq has no per-player message/notification *collection*;
...
- portaliq installed (discovery + rendering side); Larpinq stays inert
  without it.
