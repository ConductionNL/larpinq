	...
	<NcDialog v-if="navigationStore.modal === 'addSkillToCharacter'"
		...
		<div v-if="!success" class="formContainer">
			...
			<NcSelect
				:disabled="loading"
				label="Vaardigheid *"
				inputLabel="Vaardigheid *"
				:options="skills"
				:value.sync="selectedSkill"
				option-label="name"
				option-value="id"
				required
			/>
		...
		<template #actions>
			...
			<NcButton v-if="!success" :disabled="loading" type="primary" @click="addSkillToCharacter()">
				<template #icon>
...
export default {
	...
	methods: {
		async addSkillToCharacter() {
			this.loading = true
			try {
				if (!characterStore.characterItem.skills) {
					characterStore.characterItem.skills = []
				}
				characterStore.characterItem.skills.push(this.selectedSkill)
