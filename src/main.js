 ...
 * New ids are appended; existing ids are merged in place (first definition of
 * label/icon/route wins) with children unioned recursively.
 *
 * @param {Array<object>} target The accumulated menu (mutated in place).
 * @param {Array<object>} incoming Menu items from a fragment or relocation.
 * @return {void}
 */
 ...
 *
 * A relocated GROUP dissolves: its children merge into the target and the shell
 * is dropped. A relocated LEAF moves under the target group. Unknown source ids
 * are inert; a missing target group keeps the entry at the top level so nothing
 * silently disappears. Runs in passes until stable.
 *
...
function applySettingsSection(menu, settingsIds) {
	...
	const strip = (nodes) => nodes.reduce((acc, n) => {
		if (want.has(n.id)) {
			// Flatten: the settings foldout has no nested groups, so drop any
			// children and force the section tag.
			const leaf = { ...n, section: 'settings' }
 ...
 * other top-level keys from the base are preserved. After merging, the
 * src/menu-layout.json relocations / removals / settingsSection are applied to
