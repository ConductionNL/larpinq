	...
	<div class="detailContainer">
		<div id="app-content">
			<div>
				<div class="head">
					<h1 class="h1">
						{{ objectStore.getActiveObject('skill').name }}
					</h1>
				...
				<NcNoteCard v-if="objectStore.getActiveObject('skill').notice" type="info">
					{{ objectStore.getActiveObject('skill').notice }}
				</NcNoteCard>
				<div class="detailGrid">
					<div>
						<b>Type:</b>
						<span>{{ objectStore.getActiveObject('skill').type }}</span>
					</div>
					<div>
						<b>Niveau:</b>
						<span>{{ objectStore.getActiveObject('skill').level }}</span>
					</div>
				</div>
				<span>{{ objectStore.getActiveObject('skill').description }}</span>
				<div class="tabContainer">
					<ObjectTabs
						type="skill"
						:object="objectStore.getActiveObject('skill')" />
				</div>
			</div>
		</div>
	</div>
