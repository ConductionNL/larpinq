 ...
 *
 * `SettingsLoadService` passes `appId: Application::APP_ID`, which is now
 * `larpinq`. Every schema this app already owns still carries
 * `application = 'larpingapp'`, so the pair matches nothing — and
 ...
 * `larpingapp`. The gap is fleet-wide, not specific to one app — 231 under
 * `docudesk`, 200 under `openconnector`, 180 under `procest`, 98 under
 * `decidesk`, 21 under `softwarecatalog`, 17 under `openbuild`, 2 each under
 * `planix` and `hrmq`.
 *
 ...
 *
 * FOR THIS APP THAT IS THE WHOLE SET. Both `larpingapp` slugs already have a
 * twin under `larpinq` — the fork has already happened here — so on that
 * install this step refuses both and moves neither. That is the designed
 * behaviour, not a failure: it leaves a log line naming each slug a human has
 ...
 *
 * @spec exclude No canonical spec covers the `larpingapp` -> `larpinq` schema
...
class MigrateSchemaApplicationId implements IRepairStep {
	 ...
	 * @var string
	 */
	public const OLD_APP_ID = 'larpingapp';
	 ...
	 * @var string
	 */
	public const NEW_APP_ID = 'larpinq';
	...
	public function getName(): string {
		return 'Move Larpinq schemas onto the larpinq application id';
	}//end getName()
